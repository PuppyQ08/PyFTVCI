        ...
        with open(filepath) as f:
            ...
            for idx in range(FCstartidx+1, len(flines)):
                ...
                if (leng == 4):
                    ...
                    temp1 = float(tl[0])*6*math.sqrt(w_omega[int(tl[1])-1]*w_omega[int(tl[2])-1]*w_omega[int(tl[3])-1])
                ...
                if (leng == 5):
                    ...
                    temp2 = float(tl[0])*6*math.sqrt(w_omega[int(tl[1])-1]*w_omega[int(tl[2])-1]*w_omega[int(tl[3])-1]*w_omega[int(tl[4])-1])
...
def Parallel_VCI(maxn,Lambd,Temprt):
    calVCI = 0
    vcirun = VCIthermo(Lambd,Temprt,maxn,calVCI)
    np.save("../data/Temprtgrid_n_"+str(maxn-1)+".npy",vcirun.thermoresults)
...
#print((Energylist[1] -Energylist[0])*219474.63)
#print((Energylist[2] -Energylist[0])*219474.63)
#print((Energylist[3] -Energylist[0])*219474.63)
#print((Energylist[4] -Energylist[0])*219474.63)
#print((Energylist[5] -Energylist[0])*219474.63)
...
#maxnlist = [9,13,17,21,25]
#procs = []
#for ii in range(len(maxnlist)):
#    proc = Process(target = Parallel_VCI, args= ([maxnlist[ii],Lambd,Temprt]))
#    procs.append(proc)
#    proc.start()
#for procc in procs:
#    procc.join()
